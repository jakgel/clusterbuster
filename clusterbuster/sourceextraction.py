...
def RelicExtraction(image, z, contourMask = [], Imcenter=False, GCl=False, dinfo=False, rinfo= False, faintexcl=False, subtracted=False, term=False, cnt_minlength=5, gather_histo_information=True):
    ...
    if term:
        print('###==== Sub-Step X.1: Executing RelicExtraction(2d-numpy array) ====###')


    relics = []


    ## Way of extracting relics: Relic detection by simple thresholding
    (contours, hierarchy) = maput.GetThreshContours(image, dinfo.limit, rinfo.cnt)

    for jj, cnt in enumerate(contours): 
        if term:
            print('%6i out of %6i: len(cnt)=%6i' % (jj+1, len(contours), len(cnt)))
        ...
        if gather_histo_information:
            ...
            distance = np.sqrt(np.add(np.power(np.abs(GCl.RA.value-RA_arr) * np.cos(Dec_arr*np.pi/180), 2),
                                       np.power(np.abs(Dec_arr-GCl.Dec.value), 2))) * GCl.cosmoPS * 3600      #2D-array in kpc, np.linalg.norm( [GCl.RA-RA, Dec-GCl.Dec] ) * GCl.cosmoPS *3600
            angle = np.mod(np.arctan2(-(GCl.RA.value-RA_arr)*np.cos(Dec_arr*np.pi/180), Dec_arr-GCl.Dec.value)
                           + 0.5*np.pi, 2.*np.pi)   #2D-array in kpc, np.linalg.norm( [GCl.RA-RA, Dec-GCl.Dec] ) * GCl.cosmoPS *3600
            weights = region[rel]
        ...
        relic = cbclass.Relic(rinfo, dinfo, RA, Dec, LAS, area, GCl=GCl, alpha=rinfo.alpha, alphaFLAG=rinfo.alphaFLAG,
                              theta_elong=theta_elong, cov=cov, cnt=cnt, F=flux, F_ps=flux_ps, Dproj_pix=Dproj_pix,
